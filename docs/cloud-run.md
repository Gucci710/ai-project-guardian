...
`/`でAgent Guardianの設計診断・制限・再検証を実行します。[起動審査の設計と検証範囲](launch-protocol.md)も参照してください。
...
Cloud Shellの「ウェブでプレビュー」からポート8080を開きます。主画面の危険例で起動審査を開始し、制限付き許可後に下書き作成・送信拒否・削除拒否を確認してください。プロキシ実行者にはCloud Run Invoker権限が必要です。
