...
export type Result = { input: string; model: string; diagnosis: Diagnosis; risk: number; coverage: number; repair?: Repair; checks: Check[]; draft?: string; decision: "BLOCKED" | "UNVERIFIED" | "LIMITED" | "DESIGN_ONLY" | "NEEDS_INPUT"; audit: Audit[] };
...
export const workSchema = object({ calls: { type: "array", minItems: 3, maxItems: 8, items: object({ tool: str, target: str, content: str }) } });
