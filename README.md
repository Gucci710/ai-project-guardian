...
`.env.local`に`GEMINI_API_KEY`を設定し、`npm run dev`で起動してください。既存の`.env.local`がある場合は上書きせず必要な変数のみ追加します。ブラウザーで[ローカル画面](http://127.0.0.1:3000)を開きます。VS Codeでは「実行とデバッグ」から `Guardian: ローカル画面を開く` も利用できます。
...
API利用制限（429）では同じモデルを再試行せず、`GEMINI_FALLBACK_MODELS`（既定`gemini-3.5-flash-lite,gemini-3.1-flash-lite`）の次のモデルへ即時に切り替えます。一時障害では再試行後に切り替えます。変更はログに表示し、以降のAgentと承認後の再開でも使用モデルを保持します。空文字に設定すると切替を無効化できます。すべて使えない場合はエラーで停止します。認証・入力不備は切替で隠しません。従来の単一値`GEMINI_FALLBACK_MODEL`も利用できます。
...
各Gemini呼び出しは90秒タイムアウト。408・5xxの一時エラーのみモデルごとに1回再試行し、主モデルと代替モデルで合計最大4回までです。429では待たずに代替モデルへ切り替えます。全ワークフローは約28分で停止します。停止後は次のAgentを呼びませんが、送信済みのGemini処理・使用量を取り消すことはできません。
